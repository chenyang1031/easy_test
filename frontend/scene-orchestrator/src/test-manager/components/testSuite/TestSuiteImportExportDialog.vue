<template>
  <el-dialog
    v-model="visible"
    :title="activeTab === 'export' ? '导出测试套件' : '导入测试套件'"
    width="760px"
    :close-on-click-modal="false"
    :append-to-body="true"
    modal-class="ts-import-export-dialog-modal"
    destroy-on-close
    @close="handleClose"
  >
    <!-- Tab -->
    ...
    <div v-if="activeTab === 'export'" class="export-section">
      ...
      <div class="mb-3">
        ...
        <div class="text-muted small">
          <template v-if="selectedSuiteIds.length">已选中 <strong>{{ selectedSuiteIds.length }}</strong> 个套件</template>
          <template v-else>将导出当前项目下所有套件</template>
        </div>
      </div>
        ...
        <div class="mb-3">
          ...
          <el-upload ref="uploadRef" :auto-upload="false" :limit="1" :on-change="onFileChange" :on-exceed="onExceed" accept=".json,.yaml,.yml" drag>
            <div class="upload-area">
              ...
              <div class="upload-text">支持 JSON / YAML 格式的测试套件导出文件，拖到此处或点击上传</div>
            </div>
          </el-upload>
        </div>
      ...
      <div v-else class="import-step2">
        <div class="preview-summary mb-3">
          ...
          <div class="d-flex gap-3 flex-wrap">
            <span>套件分组 <strong>{{ importPreviewData.summary.test_suite_groups }}</strong> 个</span>
            ...
            <span>用例 <strong>{{ importPreviewData.summary.test_cases }}</strong> 个</span>
            <span>套件 <strong>{{ importPreviewData.summary.test_suites }}</strong> 个</span>
          </div>
        </div>
        ...
        <div v-if="importResult" class="alert alert-success py-2 small mb-0">
          导入完成！
          套件分组：创建 {{ importResult.test_suite_groups.created }}，跳过 {{ importResult.test_suite_groups.skipped }}；
          用例分组：创建 {{ importResult.test_case_groups.created }}，跳过 {{ importResult.test_case_groups.skipped }}；
          环境：创建 {{ importResult.environments.created }}，更新 {{ importResult.environments.updated }}，跳过 {{ importResult.environments.skipped }}；
          用例：创建 {{ importResult.test_cases.created }}，更新 {{ importResult.test_cases.updated }}，跳过 {{ importResult.test_cases.skipped }}；
          套件：创建 {{ importResult.test_suites.created }}，更新 {{ importResult.test_suites.updated }}，跳过 {{ importResult.test_suites.skipped }}
        </div>
      </div>
    </div>
...
import { testSuiteImportExportApi } from "../../api/index.js";

const props = defineProps({
  modelValue: { type: Boolean, default: false },
  projectId: { type: [String, Number], default: "" },
  selectedSuiteIds: { type: Array, default: () => [] },
});
...
const totalConflicts = computed(() => {
  ...
  return (
    (c.test_suite_groups?.length || 0) +
    (c.test_case_groups?.length || 0) +
    (c.environments?.length || 0) +
    (c.test_cases?.length || 0) +
    (c.test_suites?.length || 0)
  );
});
...
const allConflicts = computed(() => {
  ...
  const c = importPreviewData.value.conflicts;
  const list = [];
  (c.test_suite_groups || []).forEach((g) => list.push({ typeName: "套件分组", tagType: "warning", name: g.name, reason: g.reason }));
  (c.test_case_groups || []).forEach((g) => list.push({ typeName: "用例分组", tagType: "warning", name: g.name, reason: g.reason }));
  (c.environments || []).forEach((e) => list.push({ typeName: "环境", tagType: "info", name: e.name, reason: e.reason }));
  (c.test_cases || []).forEach((tc) => list.push({ typeName: "用例", tagType: "info", name: tc.name, reason: tc.reason }));
  (c.test_suites || []).forEach((s) => list.push({ typeName: "套件", tagType: "danger", name: s.name, reason: s.reason }));
  return list;
});
...
async function doExport() {
  ...
  exportError.value = "";
  try {
    const suiteIds = props.selectedSuiteIds.length ? props.selectedSuiteIds : null;
    const res = await testSuiteImportExportApi.export(props.projectId, suiteIds, exportFormat.value);
    ...
    a.href = url;
    a.download = res.filename || `test_suites.${exportFormat.value}`;
...
async function doPreview() {
  ...
  importError.value = "";
  try {
    const data = await testSuiteImportExportApi.previewImport(selectedFile.value, props.projectId);
...
async function doConfirm() {
  ...
  try {
    const res = await testSuiteImportExportApi.confirmImport({
...
<style>
.ts-import-export-dialog-modal { z-index: 9999 !important; isolation: isolate; }
</style>
