  ...
  <div class="group-list">
    ...
    <div class="page-header">
      <div class="page-header-left">
        <h2 class="page-title">测试用例分组</h2>
        <span class="page-subtitle">管理测试用例的分层结构</span>
      </div>
        ...
        <template #default="{ data }">
          <div class="tree-node" :style="{ borderLeftColor: getLevelColor(data._depth || 0) }">
            ...
            <div class="tree-node-meta">
              <el-tag size="small" effect="plain" round>{{ data.test_case_count || 0 }} 用例</el-tag>
              <el-tag v-if="data.sub_group_count" size="small" type="warning" effect="plain" round>{{ data.sub_group_count }} 子分组</el-tag>
            </div>
            <div class="tree-node-actions">
              <el-tooltip content="查看用例列表" placement="top">
                <el-button link type="primary" size="small" @click.stop="$router.push(`/test-cases?group=${data.id}`)">
    ...
    <GroupFormDialog v-model="dialogVisible" :editing="editingGroup" :project-id="filterProjectId" :groups="store.flatList" type="testCase" @saved="onSaved" />
  </div>
</template>
...
const store = useTestCaseGroupStore()
