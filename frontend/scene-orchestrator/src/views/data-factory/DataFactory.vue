...
import axios from '../../utils/axios-shim.js'
...
const fetchCategories = async () => {
  try {
    const response = await axios.get('/api/v1/data-factory/categories/')
...
const executeTool = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/v1/data-factory/', {
...
const debouncedFetchHistory = debounce(async () => {
  ...
  try {
    const response = await axios.get('/api/v1/data-factory/', {
      params: {
...
const fetchStatistics = async () => {
  ...
  try {
    const response = await axios.get('/api/v1/data-factory/statistics/', {
      params: {
        _t: Date.now()
      }
    })
...
const deleteRecord = async (record) => {
  try {
    ...
    const response = await axios.delete(`/api/v1/data-factory/${record.id}/`)
...
const getImageUrl = (url) => {
  ...
  return `/api/v1/data-factory/download_static_file/?filename=${url.split('/').pop()}`
}
...
const downloadImage = (result) => {
  ...
  const downloadUrl = `/api/v1/data-factory/download_static_file/?filename=${filename}`
